  ...
  $.lrworkflow = {
      ...
      addNode: function ($workArea, dfop, node, isold) {
          ...
          if (node.state != undefined && (node.type == 'startround' || node.type == 'auditornode' || node.type == 'stepnode' || node.type == 'confluencenode')) {
              $node.css({
                  'padding-left': '0',
                  'color': '#fff'
              }).find('.lr-workflow-nodeico').remove();
      ...
      initNodeEvent: function ($workArea) {
          ...
          if (!dfop.isPreview) {
              ...
              //绑定鼠标覆盖/移出事件
              $workArea.delegate(".lr-workflow-node", "mouseenter", {
                  $workArea: $workArea
              }, function (e) {
              ...
              $workArea.delegate(".lr-workflow-node", "mouseleave", {
                  $workArea: $workArea
              }, function (e) {
                  var dfop = e.data.$workArea[0].dfop;
                  if (dfop.currentBtn != "direct") return;
              ...
              $workArea.delegate(".lr-workflow-nodespot", "mouseleave", {
                  $workArea: $workArea
              }, function (e) {
                  var dfop = e.data.$workArea[0].dfop;
                  if (dfop.currentBtn != "direct") return;
              ...
              //绑定连线时确定结束点
              $workArea.delegate(".lr-workflow-nodespot", "mouseup", {
                  $workArea: $workArea
              }, function (e) {
                  ...
                  if (lineStart) $.lrworkflow.addLine(dfop, {
                      id: generateUUID(),
                      from: lineStart.id,
                      to: nodeData.id,
